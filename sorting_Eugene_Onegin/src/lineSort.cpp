...
static int CompareFlippedLines(const void *value1, const void *value2)
{
    assert(value1 && value2);
    ...
    while(index1 && index2)
    {
        while(index1 && !IsRuLetter(string1[index1]))
            index1--;
        while(index2 && !IsRuLetter(string2[index2]))
            index2--;
...
char SortFlippedLines(const char **lines, size_t count)
{
    ...
    MySort(lines, count, sizeof(char *), CompareFlippedLines);
